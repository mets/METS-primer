...
When a file contains other files, such as with archive file formats like `.zip`,  using child `<file>` elements is preferred (see also [[Handling 'wrapper' file formats]]). Use `<stream>` when the subsidiary streams are logically separate entities but not individual files in their own right. 
## Example
