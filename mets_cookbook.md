---
title: METS Cookbook
nav_order: 4
has_toc: false
---

# METS Cookbook

Examples and descriptions of how to accomplish various tasks in METS using the available elements and attributes as well as how to relate together various parts of the METS document and the components digital object it describes.

## METS Header
* [Alternative identifiers for a digital object](cookbook/altRecordID.md)
* [Recording who created or modified a METS document](cookbook/agent.md)
## Metadata
* [Describing the digital object with metadata](cookbook/md.md)
* [Descriptive metadata - embedding binary MARC records](cookbook/binary_marc.md)
* [Digital provenance metadata -- PREMIS](cookbook/premis.md)
* [Describing characteristics of the source material](cookbook/source_metadata.md)
* [Including descriptive metadata with MODS](cookbook/mods.md)
* [Intellectual property rights metadata](cookbook/mets_rights.md)
* [Technical metadata for images](cookbook/technical_metadata.md)
* [Referring to external metadata](cookbook/mdRef.md)
* [Embedding metadata in METS](cookbook/mdWrap.md)
## Content Files
* [Describing digital object content](cookbook/file.md)
* [File section example - audio and transcription](cookbook/fileSec_example_audio.md)
* [File section example - multiple image representations](cookbook/fileSec_example_images.md)
* [Grouping related files](cookbook/fileGrp.md)
* [Handling 'wrapper' file formats](cookbook/transformFile.md)
* [Representing parts of files](cookbook/stream.md)
* [Referencing external files](cookbook/FLocat.md)
* [Embedding file content](cookbook/FContent.md)
## Embedding content and metadata
* [Embedding file content](cookbook/FContent.md)
* [Embedding metadata in METS](cookbook/mdWrap.md)
* [Embedding XML content in METS](cookbook/xmlData.md)
* [Embedding binary data in METS](cookbook/binData.md)
## Structuring and Linking
* [Referencing METS elements from external documents](cookbook/external_idref.md)
* [Referencing external files](cookbook/FLocat.md)
* [Referring to Parts of XML Documents](cookbook/id_idrefs.md)
* [Referring to elements within METS](cookbook/idrefs_within_mets.md)
* [Referring to external metadata](cookbook/mdRef.md)
* [Referring to parts of external XML documents](cookbook/begin_end_betype.md)
* [Linking to external resources from METS](cookbook/locref.md)
* [IDREF linking across different namespaces](cookbook/idref_namespaces.md)
